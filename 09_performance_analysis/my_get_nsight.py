...
def parse_metrics(result, metrics_list):
    # Parse the stdout from ncu to extract metric values
    lines = result.stdout.splitlines()
    metric_values = [""] * (len(metrics_list))  # Initialize empty metric values list
    ...
    for line in lines:
        ...
        for metric_index, metric_name in enumerate(metrics_list, start=1):
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Программа для анализа CUDA-kernel с помощью Nsight Compute"
    )
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Имя CUDA-kernel, который будет анализироваться",
    )

    args = parser.parse_args()

    # TODO: добавить аргумент для считывания списка метрик из файла

    # TODO: добавить аргументы для параметров цикла
    ...
    with open(output_file, mode="w", newline="") as csvfile:
        ...
        for block_size in range(32, 1024, 32):
            try:
                # Run the ncu command
                command = [
                    "ncu",
                ...
                # Parse the stdout from ncu to extract metric values
                metric_values = parse_metrics(result, metrics_list)
